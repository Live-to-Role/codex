...
**STATUS: IMPLEMENTED** - See changes in `backend/apps/api/validators.py`

## Issue
Grimoire contributions are being rejected because the Codex API doesn't accept the `publisher` field.
...
```json
{
  ...
  "details": {"data": "Unknown fields not allowed: publisher"}
}
```

## Required Change
Update the Codex contribution data serializer/validator to accept the `publisher` field.
